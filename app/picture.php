...
class picture extends Model
{
    ...
    public static function add(String $path)
    {
        if (Storage::exists($path)) {
            ...
            $picture->save();
            $picture->writeExif();
            Watchdog::watch($picture->postProcessJpeg(), 'Post processing');
            return [true, $picture];
        } else {
            return [false, null];
        }
    ...
    public function postProcessJpeg()
    {
        ...
        if (parameters::get('jpeg rotate', 0) !== 0) {
            $GD = jpeg::rotate($GD, parameters::get('jpeg rotate', 0));
            $postProccess = true;
        }

        if (parameters::get('jpeg add Date', true)) {
            $GD = jpeg::addDateTimeLoc($GD, $this->date);
            $postProcess = true;
        }
        ...
        if ($postProcess) {
            ...
            if (!Storage::exists($this->filename.'-NOEXIF')) {
                return false;
            }
